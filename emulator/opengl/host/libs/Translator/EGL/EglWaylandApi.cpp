...
int (*m_oldErrorHandler) (wl_display *, void *);
static android::Mutex s_lock;
static int errorHandlerProc(EGLNativeDisplayType dpy,void* event);
...
int ErrorHandler::errorHandlerProc(EGLNativeDisplayType dpy,void* event){
    android::Mutex::Autolock mutex(s_lock);
    /* krnlyng hmm */
...
EGLNativeDisplayType getDefaultDisplay() {return wl_display_connect(0);}
